...
>
> **측정 편향.** 이 표의 1.0%·16% 와 아래 2.4% 는 전부 **1800 RPM 회차**(p95 44~56초)에서 나온 값인데, 그 구간의 `e2e` 는 폴링 눈금 위에 있고 회차 끝에서 잘린다 — k6 가 남긴 건수가 DB 가 센 것의 **84.4~96.1%** 이고, **잘리는 건 제일 느린 것들**이다. 2.4% 를 읽은 두 팔은 커버리지가 84.4% 대 93.1% 로 서로 다르다. 게다가 io1024 coroutine 회차는 submit 실패 163건으로 `runs_total` 이 163 적은데도 `valid:true` 로 통과했다.
>
...
격자가 무너진 1800 회차의 e2e 최솟값이 **30,028ms** 다. 이게 양자화 없이 본 첫 값이라 오버헤드가 1초보다 훨씬 작다는 건 알 수 있는데, **그 값도 폴로 본 시각이고 6,172개 중 최솟값**이며 접수 시간이 빠져 있다. 오버헤드의 **상한이 28ms** 라는 뜻이지 28ms 라고 잰 게 아니다.
...
| **타임아웃이 상수로 들어간다** | 타임아웃 난 트랜잭션은 `MAX_WAIT_MS`(180,000) 가 그대로 트렌드에 더해진다 | 높게 |

**첫 줄이 A/B 를 직접 깬다.** 2.4% 를 읽은 coroutine 두 팔의 커버리지가 **84.4% 대 93.1%** 다. 느린 쪽을 더 많이 버린 팔과 덜 버린 팔을 비교한 것이다.

그리고 **그 회차 중 하나는 애초에 정상이 아니었다.** io1024 coroutine 은 `runs_total` 이 7,037 로 다른 회차(7,200)보다 163 적고, k6 check 실패가 정확히 163건이다 — submit 이 202 를 못 받았다는 뜻이다. 그런데 회차는 `valid:true` 로 통과했고 표 어디에도 안 남는다.
