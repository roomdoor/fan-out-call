...
>
> **측정 해상도.** `e2e` 는 k6 가 폴링으로 확인한 시각이고, 대기 간격이 5초에서 멈춘다. 이 표의 1.0%·16% 와 아래 2.4% 는 전부 **1800 RPM 회차**(p95 44~56초)에서 나온 값인데, 하필 그 구간은 요청 왕복이 초 단위라 간격이 `5초 + 왕복` 으로 벌어진다. 게다가 **비교하는 두 팔의 왕복이 서로 다르다** — 커넥션 풀 쪽은 985/1,148ms, IO 워커 쪽은 1,798/1,968ms 다. 눈금 간격이 팔마다 달라진 상태에서 1.0%·16% 를 읽은 것이다.
>
...
여기에 물리 바닥이 겹친다. 느린 은행이 30초를 자므로 실제 완료는 `(30,000, 그 회차의 e2e 최댓값]` 안이다 — 480 이면 **2.2초**, coroutine 1400 이면 **3.0초** 폭이다. 어느 트랜잭션이 그보다 더 걸렸으면 16번째 폴로 넘어가 폴 수가 15.0 을 넘었을 것이다. 안 넘었다.

**그래서 이 구간의 콜드 비용은 트랜잭션당 2~3초 아래로 묶인다.** 31초 대비 7~10% 다. 원래 "여기서는 워밍업이 문제가 안 된다" 던 결론이 **낮은 rate 에서는 대체로 맞았다** — 다만 이유가 내가 적었던 것("하한에 붙어 있어서")이 아니라 이거다.

**그리고 그 "이론 하한 31초" 자체가 눈금이었다.** 14번째 폴이 26,333 에 떨어지므로 **31,333 아래는 애초에 관측이 안 된다.** 31.4초가 하한과 일치한 게 아니라, 그 위쪽 첫 눈금이 거기였을 뿐이다. 하한에 얹었다던 "오버헤드 1초" 는 재본 적이 없다. 격자가 무너진 1800 회차의 e2e 최솟값 **30,028ms** 가 양자화 없이 본 첫 값이고, 그게 맞다면 실제 오버헤드는 **28ms** 다.
...
**덤으로 e2e 는 접수 시간을 아예 안 센다.** `pollUntilTerminal` 이 `startTime` 을 submit 이 202 를 돌려준 **뒤에** 잡는다. 1800 에서는 그 submit 자체가 느려서, 빠진 양이 부하에 따라 달라진다. 이건 위의 30,028ms 와는 다른 얘기다 — 그쪽은 양자화가 풀린 것이고, 이쪽은 시작점이 뒤로 밀린 것이다. 두 편향 모두 **e2e 를 실제보다 낮게** 만든다.
