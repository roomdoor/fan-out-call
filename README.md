...
| **JVM 워밍업** | `bench.sh` 가 회차마다 게이트웨이를 재기동한다. 포화 구간에서는 초반 지연이 대기열을 만들고 4분 안에 안 빠질 수 있다. 다만 25초를 만들기엔 약하다 — 아래 방법론 참고. **미검증** |
...
### 회차마다 게이트웨이를 재기동한다 — 여기서는 워밍업이 문제가 안 된다

`bench.sh` 는 회차마다 게이트웨이를 내렸다 올린다(모드와 풀 크기가 기동 인자라 필요하다). 그러면 **매 회차가 갓 뜬 JVM 을 잰다.** JVM 은 처음에 코드를 해석하다가 자주 도는 부분만 기계어로 컴파일하고, 클래스도 처음 쓸 때 로딩한다.

**이 측정에서는 영향이 없다.** 재는 값이 워밍업 비용보다 두 자릿수 크기 때문이다.

- 트랜잭션 하나의 이론 하한이 **31초**다. 거기에 수백 ms 가 얹혀도 1% 미만이다
- 실측 p95 가 그 하한보다 **1.3% 위에 붙어 있다**(480 RPM 31,396ms). 왜곡이 숨을 자리가 없다 — 있었다면 하한에서 한참 떠 있어야 한다
- 부하를 3.3배 올리는 동안 p95 가 2%밖에 안 움직였다. 콜드였다면 회차마다 일정한 offset 으로 나타났을 것이다
- 이 워크로드는 **대부분 기다린다**. 은행 호출 50개가 7~30초씩 자는 동안 CPU 가 놀고, JIT 이 그 틈에 끝난다

응답이 밀리초 단위인 서비스였다면 반대다 — 같은 조건에서 워밍업이 측정값을 통째로 삼킨다. 자매 저장소 [`check-in-event`](https://github.com/roomdoor/check-in-event) 에서 실제로 그 일이 있었고, 같은 부하에서 p95 가 686배 차이났다.

**포화 구간(1800 RPM)은 예외일 수 있다.** 대기열이 있으면 초반의 작은 지연이 증폭된다. 미해결에 후보로 올려뒀다.
