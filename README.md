...
논블로킹 두 모드는 1,800 RPM에서도 7,200건을 **전부 처리했다.** 거부가 아니라 지연이 늘었을 뿐이다(31.4초 → 56.3 / 44.6초). 위 표의 1,600은 "31.4초라는 같은 폴링 눈금에 머무는" 한계인데, 엄밀히는 **1,400까지**다 — coroutine 은 1,600 에서 이미 일부가 다음 눈금으로 넘어갔다. 그 31.4초가 지연이 아니라 측정 눈금이라는 것도 2장 표 밑에 적었다.
...
IO 워커를 512로 고정하고 RPM만 올리며 e2e p95를 봤다. 물리 하한은 **30초**(느린 은행의 sleep)이고, 아래 31.4초는 그 위 첫 폴링 눈금이다 — 자세한 건 표 밑 주의 참고.
...
**1400까지 같은 폴링 눈금에 머물다가 그 위에서 꺾인다.** 전 구간 거부 0이다. coroutine 은 1600 에서 이미 일부가 다음 눈금으로 넘어갔고(32,044ms), webclient 는 1600 까지 버틴다.

> **이 표는 지연이 아니라 폴링 눈금을 읽은 값에 가깝다.** `e2e` 는 k6 가 폴링으로 확인한 시각이고 간격이 5초에서 멈춘다 — **1400 까지는** k6 가 기록한 트랜잭션이 전부 같은(15번째) 폴에서 끝나므로 31,396 과 31,392 의 차이는 왕복 시간 차이지 처리 시간 차이가 아니다. 1600 은 webclient 만 그렇고 coroutine 은 일부가 16번째 폴로 넘어갔다(15.0003회, 최댓값 38,188ms). 1800 행은 눈금 자체가 무너진 구간이다. 방법론의 [회차마다 게이트웨이를 재기동한다](#회차마다-게이트웨이를-재기동한다--워밍업-영향은-안-쟀다) 참고.
...
**두 논블로킹 구현은 1400까지 구분되지 않는다.** 1200에서 7ms 차이인데, 그 7ms 는 같은 폴에서 읽은 왕복 차이라 처리 시간 차이로 볼 수 없다.

**처음 갈리는 건 1600 이다.** coroutine 만 일부가 다음 폴로 넘어갔다(32,044 대 31,632ms) — 이 문서의 눈금 기준으로는 그게 첫 **관측 가능한** 분리다. 1800 에서 차이가 커진다(webclient가 21% 빠름).
...
>
> **측정 편향.** 이 표의 1.0%·16% 와 아래 2.4% 는 전부 **1800 RPM 회차**(p95 44~56초)에서 나온 값인데, 그 구간의 `e2e` 는 폴링 눈금 위에 있고 회차 끝에서 잘린다 — k6 가 남긴 건수가 DB 가 센 것의 **84.4~96.1%** 이고, **잘리는 건 제일 느린 것들**이다. 2.4% 를 읽은 두 팔은 커버리지가 **84.4% 대 88.7%** 로 서로 다르다(같은 분모 7,201 기준). 게다가 io1024 coroutine 회차는 submit 실패가 163건이고 `runs_total` 이 7,037 로 형제 회차보다 적은데도 `valid:true` 로 통과했다.
>
...
k6는 `DURATION` 에서 멈추지만 트랜잭션 하나가 최소 30초는 걸린다(느린 은행의 sleep. 거부가 섞이면 그 호출이 즉시 끝나 더 짧아진다). 막바지에 넣은 건들이 끝날 때까지 기다린 뒤에 센다. 짧게 기다리면 처리량이 낮게 나오고, **부하가 셀수록 많이 빠져 천장이 실제보다 낮아 보인다.**
...
(모드가 바뀔 때는 달라질 수 있다. `pool_args_for()` 는 `POOL` 이 설정돼 있고 모드가 `async-threadpool` 일 때만 인자를 붙이므로, 그 둘이 겹치는 설정에서 `MODES` 에 다른 모드를 같이 넣으면 명령줄이 갈린다. 지금 커밋된 설정 중에 그런 건 없다.)
...
**31초 근처의 측정 해상도가 5초다.** 실제로 26.4초에 끝난 트랜잭션도, 31.3초에 끝난 것도 15번째 폴에서야 발견된다.

다만 **눈금이 고정은 아니다.** 폴마다 HTTP 왕복이 `sleep` 위에 얹히므로 15번째 폴은 `31,333 + 누적 왕복` 에 떨어진다. 실측 최댓값이 480 에서 32,217(+884ms), coroutine 1400 에서 32,968(**+1,635ms**)이다. 눈금은 위로 밀린다.

**1400 까지는 모든 회차가 눈금 하나에 뭉친다.** 폴 수를 기록 건수로 나누면 정확히 15.000 이다 — 두 모드의 480·960·1200·1400 과 webclient 1600, 아홉 회차 전부. 그 회차들의 e2e 최솟값이 31,333 위이고 최댓값이 36,333 아래라, **기록된 트랜잭션이 전부 15번째 폴에서 끝났다**는 뜻이다. 즉 각각은 14번째 폴(26,333) 뒤에, 15번째 폴이 실제로 뜬 시각(`31,333 + 누적 왕복`) 전에 끝났다.

**그러니 "이론 하한 31초" 자체가 눈금이었다.** 물리 바닥은 느린 은행의 **30초**고, 14번째 폴이 26,333 에 떨어지므로 30초를 다 자는 트랜잭션은 **31,333 아래로 관측될 수가 없다.** 31.4초가 하한과 일치한 게 아니라 그 위 첫 눈금이 거기였다. 하한에 얹었다던 "오버헤드 1초" 는 재본 적이 없다.

(거부가 섞이면 다르다. 거부된 호출은 즉시 끝나 async-threadpool 회차의 e2e 최솟값이 16,361~16,383ms — 12번째 폴 눈금이다.)

**같은 이유로 이 구간의 콜드 비용도 묶인다.** 실제 완료가 `(30,000, 그 회차 최댓값]` 안이므로 폭이 2~3초다. 더 걸렸으면 16번째 폴로 넘어가 비율이 15.0 을 넘었을 것이다 — coroutine 1600 이 실제로 그렇다(15.0003, 최댓값 38,188ms). 원래 "여기서는 워밍업이 문제가 안 된다" 던 결론이 **낮은 rate 에서는 대체로 맞았다.** 다만 이유가 내가 적었던 것("하한에 붙어 있어서")이 아니라 이거다.

**1800 에서는 눈금이 무너진다.** 완료 시각이 30~69초로 흩어지면서 폴 수 비율이 **13.78**(84,911/6,163)로 정수가 아니게 된다 — 트랜잭션마다 다른 폴에서 끝난다는 뜻이다. **하필 무릎과 그 원인 후보를 지운 A/B 들이 거기 산다.**

그리고 그 구간에는 `e2e` 에 편향이 둘 더 얹힌다.

| 편향 | 실측 | 방향 |
| --- | --- | --- |
| **끝에서 잘린다** | k6 가 e2e 를 남긴 건수가 DB 가 센 것의 **84.4~96.1%** 다. `gracefulStop` 에 잘린 iteration 은 지표에 안 들어가는데, 잘리는 건 **제일 늦게 도착한 것들**이다 | 낮게 |
| **접수 시간이 빠진다** | `startTime` 을 submit 이 202 를 돌려준 뒤에 잡는다 | 낮게 |

**2.4% 는 해석할 수 없는 값이다.** 그걸 읽은 coroutine 두 팔의 커버리지가 **84.4% 대 88.7%** 로 다르고(같은 분모 7,201 기준), 게다가 **io1024 쪽 회차는 애초에 정상이 아니었다** — k6 check 실패가 163건이고 `runs_total` 이 7,037 이다(형제 회차는 7,200~7,201, 회차마다 ±1 흔들린다). submit 이 202 를 못 받았다는 뜻이고, **그 팔은 부하를 2% 덜 받은 것**이다. 2.4% 라는 차이와 같은 방향의 교란이다. 그런데도 회차는 `valid:true` 로 통과했고 표 어디에도 안 남는다.
...
| 1800 RPM 무릎, 그리고 그 원인 후보를 지운 A/B | **모른다.** 전부 n=1 콜드 회차이고, 그 구간은 눈금이 무너진 데다 팔마다 잘린 양이 다르다 |
...
**워밍업** — 논블로킹 모드는 RPM 단계들을 **같은 JVM 위에서** 연속으로 돌릴 수 있다. 인자가 같아서 재기동이 인자 때문에 필요하진 않다. 다만 공짜는 아니다 — `bench.sh` 가 회차마다 `docker logs` 를 통째로 떠서 `/var/log/bench/` 에 남기므로(`run_id` 의 `/` 를 `_` 로 바꾼 이름, 예: `coroutine_default_rpm1800_rep1.log`), JVM 을 이어 쓰면 뒤 회차 로그에 앞 회차가 전부 섞인다. 방법론의 "표에서 이상이 보이면 A 의 로그를 본다" 가 회차 단위로 안 먹는다.

**단 `async-threadpool` 은 안 된다.** 위에 적은 대로 4,096 스레드가 회차 뒤에도 살아 있어서, 안 내리면 다음 회차가 이미 만들어진 풀로 시작한다. 그러면 **거부 건수(317/740/1641/3409)가 무효가 된다** — 큐가 차는 속도가 달라지기 때문이다. `pool ÷ 9` 의 경계 자체는 정상 상태 용량이라 그대로다(위 표와 같은 구분이다). 그쪽은 재기동을 유지하고 버리는 부하를 앞에 흘려보내야 한다. 자매 저장소 [`check-in-event`](https://github.com/roomdoor/check-in-event) 가 그 방식을 쓴다.
