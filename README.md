...
| **coroutine** | 이벤트 루프 + 저장만 IO **512** | 거부 0, 처리율 480.3/분 | **1,600 RPM** |
| **webclient** | 이벤트 루프 + 저장만 IO **512** | 거부 0, 처리율 480.3/분 | **1,600 RPM** |
| sequential | 1 | — | anti-pattern 시연용 |

**스레드를 8분의 1만 쓰고 부하를 전부 받아냈다.** async-threadpool 은 풀 4,096, coroutine 은 IO 워커 512다.

이 8배는 설정한 스레드 수의 대비일 뿐이다. coroutine 의 512는 은행 호출을 떠받치는 풀이 아니라 결과 저장용이고, async-threadpool 도 저장에는 같은 512를 쓴다. 실제로 갈리는 건 **대기 중에 스레드를 붙잡느냐**다 — async-threadpool 은 `runBlocking` 이 응답이 올 때까지 풀 스레드를 점유하고, coroutine 은 그 대기에 스레드를 하나도 안 쓴다.
...
DB 쪽을 의심하는 이유는 이렇다. 두 모드 모두 결과 저장을 `Dispatchers.IO` 로 넘기므로, 워커가 두 배면 MySQL로 가는 동시 저장도 두 배다. 워커를 늘렸을 때 webclient 가 16.1% 느려진 것이 여기에 맞는다.

**다만 증거가 갈린다.** 같은 변경에서 coroutine 은 오히려 2.4% 빨라졌다(55,766ms → 54,419ms). n=1 이라 이 정도는 잡음일 수 있지만, 한쪽만 보고 DB로 단정할 근거는 안 된다.
...
k6는 `DURATION` 에서 멈추지만 거부가 없으면 트랜잭션 하나가 최소 31초다. 막바지에 넣은 건들이 끝날 때까지 기다린 뒤에 센다. 짧게 기다리면 처리량이 낮게 나오고, **부하가 셀수록 많이 빠져 천장이 실제보다 낮아 보인다.**
