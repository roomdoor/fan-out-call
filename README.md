...
>
> **측정 해상도.** `e2e` 는 k6 가 폴링으로 확인한 시각이고, 대기 간격이 5초에서 멈춘다. 31초 근처의 눈금이 `26,333 → 31,333 → 36,333ms` 라 **그 사이 차이는 안 보인다.** 위의 1.0%·16%·2.4% 가 전부 그 눈금 위에서 읽은 값이다.
>
...
**기동 인자 때문에 재기동해야 하는 회차는 없다.** `POOL`·`QUEUE` 는 설정에서 한 번 읽고 `pool_args` 도 모드마다 한 번 계산해 rpm·rep 루프 **밖**에 둔다. 모드 선택은 게이트웨이가 아니라 k6 로 간다(엔드포인트를 고른다). 그래서 한 번의 `bench.sh` 실행 안에서는 **모든 회차가 같은 명령줄로 다시 뜬다.** 하네스에 워밍업 단계도 없다.

(`async-threadpool` 만 인자와 무관한 이유가 하나 있다. `core-pool-size = max-pool-size` 라 4,096 스레드가 회차 뒤에도 살아 있어, 안 내리면 다음 회차가 이미 만들어진 풀로 시작한다.)
...
실측이 그대로다. 480 RPM 회차의 폴 수가 트랜잭션당 **정확히 15.0회**(28,635/1,909)이고, e2e 가 min 31,366 · med 31,374 · p95 31,396 으로 **30ms 안에 뭉쳐 있다.** 모든 트랜잭션이 같은 눈금에 찍혔다는 뜻이다.

- **"p95 가 하한 31초에 붙어 있으니 왜곡이 숨을 자리가 없다"** — 하한에 붙은 게 아니라 **눈금에 붙은 것**이다. 5초 아래의 콜드 비용은 통째로 안 보인다
- **"부하를 3.3배 올려도 p95 가 2%밖에 안 움직였다"** — 같은 눈금 안에 있으면 안 움직이는 게 당연하다. 무릎(56,344ms)도 56,333ms 눈금 바로 위다

**버티는 결론과 안 버티는 결론이 갈린다.**

| | 워밍업에 영향받나 |
| --- | --- |
| `pool ÷ 9` 천장 (거부 317/740/1641/3409) | **아니다.** 풀 크기 대 점유 시간의 산수고, 점유 시간은 mock 의 sleep 이 정한다. 거부 수는 폴링 해상도와도 무관하다 |
| 1800 RPM 무릎, 그리고 그 원인 후보를 지운 A/B | **모른다.** 전부 n=1 콜드 회차이고, 지연 차이는 5초 눈금 위에서 읽은 값이다 |

두 번째 줄이 아프다. 커넥션 풀(1.0% 차이)과 IO 워커(16% 악화 / 2.4% 개선)로 후보를 지운 판단이 **전부 같은 오염 구간 안에서 나왔다.** 지웠다고 보기 어렵다.

**고칠 자리가 둘이다.** 폴링 간격 상한을 낮춰 해상도를 올리고(5초 눈금으로는 워밍업이든 뭐든 초 단위 차이를 못 본다), 한 모드의 RPM 단계들을 **같은 JVM 위에서** 연속으로 돌리면 된다 — 인자가 같으므로 재기동이 필요 없다. 자매 저장소 [`check-in-event`](https://github.com/roomdoor/check-in-event) 는 본 측정 앞에 버리는 부하를 흘려보내는 쪽을 쓴다.
