...
| **JVM 워밍업** | `bench.sh` 가 회차마다 게이트웨이를 재기동해 **모든 회차가 콜드 JVM 이다.** 자매 저장소에서 실측된 콜드 비용이 1,474ms 로, 여기 하한 위 여유분(1,396ms)과 같은 자릿수다. **미검증** |

> **워밍업 행은 위의 네 행도 흔든다.** 커넥션 풀·IO 워커·CPU·DB 저장을 지운 판단이 전부 1800 RPM 의 n=1 콜드 회차에서 나왔다. 워밍업이 그 구간을 움직일 수 있다면 그 A/B 차이들도 같은 오염 안에 있고, "지웠다" 고 말하기 어렵다. 방법론의 [회차마다 게이트웨이를 재기동한다](#회차마다-게이트웨이를-재기동한다--워밍업-영향은-안-쟀다) 참고.
...
**재기동이 꼭 필요한 건 async-threadpool 뿐이다.** `pool_args_for()` 는 그 모드가 아니면 빈 문자열을 반환하고, 모드 선택은 게이트웨이가 아니라 k6 로 간다(엔드포인트를 고른다). coroutine·webclient 회차는 **매번 같은 명령줄로 다시 뜬다** — 필요해서가 아니라 루프 구조가 그렇다. 하네스에 워밍업 단계는 없다.

**이 측정이 워밍업에 오염됐는지는 모른다.** 처음에는 아니라고 적었는데, 근거로 든 것들이 반증이 안 된다.

- **"p95 가 하한에 붙어 있다"** — 하한 31초는 **가정이 섞인 값**이다. 진짜 바닥은 느린 은행의 30초고 나머지 1초는 재본 적 없는 오버헤드 예산이다. 실측 31,396ms 는 30,000 보다 **1,396ms 위**이고, 워밍업 비용이 있다면 정확히 그 안에 들어간다
- **"부하를 3.3배 올려도 p95 가 2%밖에 안 움직였다"** — 모든 회차가 재기동되므로 **일정한 워밍업 비용은 모든 회차에 똑같이 실린다.** 평평한 p95 는 "비용 없음"과 "매번 같은 비용"을 구분하지 못한다
- **"워밍업 비용은 수백 ms 라 두 자릿수 작다"** — 실측된 유일한 콜드 비용은 자매 저장소의 **1,474ms** 다. 31,000 대비 21배, 한 자릿수다. 그리고 수백 ms 가 아니라 초 단위다

**버티는 결론과 안 버티는 결론이 갈린다.**

| | 워밍업에 영향받나 |
| --- | --- |
| `pool ÷ 9` 천장 (거부 317/740/1641/3409) | **아니다.** 풀 크기 대 점유 시간의 산수고, 점유 시간은 mock 의 sleep 이 정한다 |
| 1800 RPM 무릎, 그리고 그 원인 후보를 지운 A/B | **모른다.** 전부 n=1 콜드 회차다 |

두 번째 줄이 아프다. 커넥션 풀(1.0% 차이)과 IO 워커(16% 악화 / 2.4% 개선)로 후보를 지운 판단이 **전부 같은 오염 구간 안에서 나왔다.** 지웠다고 보기 어렵다.

**고치는 법은 간단하다.** 한 모드의 RPM 단계들을 **같은 JVM 위에서** 연속으로 돌리면 된다 — coroutine·webclient 는 기동 인자가 같으므로 재기동이 필요 없다. 또는 본 측정 앞에 버리는 부하를 흘려보낸다. 자매 저장소 [`check-in-event`](https://github.com/roomdoor/check-in-event) 가 그 방식을 쓰고, 거기서는 같은 부하에 p95 가 **686배** 차이났다.
