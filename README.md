...
실측이 그대로다. 480 RPM 회차의 폴 수가 트랜잭션당 **정확히 15.0회**(28,635/1,909)이고, e2e 가 min 31,366 · med 31,374 · p95 31,396 으로 **30ms 안에 뭉쳐 있다.** 1600 RPM 도 여전히 15.0회(95,445/6,363)다. 모든 트랜잭션이 같은 눈금에 찍혔다는 뜻이다.

- **"p95 가 하한 31초에 붙어 있으니 왜곡이 숨을 자리가 없다"** — 하한에 붙은 게 아니라 **눈금에 붙은 것**이다. 5초 아래의 콜드 비용은 통째로 안 보인다
- **"부하를 3.3배 올려도 p95 가 2%밖에 안 움직였다"** — 480 부터 1600 까지 폴 수가 똑같이 15.0회다. 같은 눈금 위에 있으면 안 움직이는 게 당연하다

**1800 에서는 눈금마저 무너진다.** 거기서는 폴 왕복 자체가 느려져(평균 1,167ms, p95 4,068ms) 폴 수가 **13.76회**(84,911/6,172)로 정수가 아니게 된다 — 트랜잭션마다 다른 폴에서 끝난다는 뜻이다. e2e 가 min 30,028 · med 38,594 · max 69,422 로 흩어지고 어느 눈금에도 안 걸린다.

**그러니 1800 구간의 해상도는 5초보다 나쁘다.** 간격이 `5초 + 폴 왕복` 이라 평균 6.2초쯤이고, 부하에 따라 변한다. 이 구간이 하필 무릎과 그 원인 후보를 지운 A/B 들이 사는 곳이다.
...
**워밍업** — 논블로킹 모드는 RPM 단계들을 **같은 JVM 위에서** 연속으로 돌리면 된다. 인자가 같아서 재기동할 이유가 없다.
