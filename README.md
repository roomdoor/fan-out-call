...
> **이 표는 지연이 아니라 폴링 눈금을 읽은 값에 가깝다.** `e2e` 는 k6 가 폴링으로 확인한 시각이고 간격이 5초에서 멈춘다 — **1600 까지는** 기록된 트랜잭션이 사실상 전부 같은(15번째) 폴에서 끝난다. 그러니 이 표의 차이들(31,396 대 31,392, 32,044 대 31,632)은 **그 폴이 실제로 뜬 시각의 차이**, 즉 누적 왕복 차이지 처리 시간 차이가 아니다.

> coroutine 1600 만 6,362건 중 **2건**이 16번째 폴로 넘어갔다(15.0003회, 최댓값 38,188ms). p95 인 32,044ms 는 그 2건이 아니라 여전히 15번째 폴 값이다. **2건으로 모드 간 차이를 말할 수는 없다** — n=1 콜드 회차다.

1800 행은 눈금 자체가 무너진 구간이다. 방법론의 [회차마다 게이트웨이를 재기동한다](#회차마다-게이트웨이를-재기동한다--워밍업-영향은-안-쟀다) 참고.
...
**두 논블로킹 구현은 1600까지 구분되지 않는다.** 1200에서 7ms, 1400에서 62ms, 1600에서 412ms 차이인데 **전부 같은 폴에서 읽은 값**이다(31,333 위의 누적 왕복 차이). 처리 시간 차이로 볼 수 없다. 1800에서 처음 갈린다(webclient가 21% 빠름).
...
>
> **측정 편향.** 이 표의 1.0%·16% 와 아래 2.4% 는 전부 **1800 RPM 회차**(p95 44~56초)에서 나온 값인데, 그 구간의 `e2e` 는 폴링 눈금 위에 있고 회차 끝에서 잘린다 — k6 가 남긴 건수가 DB 가 센 것의 **84.4~96.1%** 이고, **잘리는 건 제일 느린 것들**이다. 2.4% 를 읽은 두 팔은 커버리지가 **84.4% 대 88.7%** 로 서로 다르다(같은 분모 7,201 기준). 더 결정적인 건 io1024 coroutine 회차가 submit 실패 163건으로 **부하를 2% 덜 받았는데도** `valid:true` 로 통과했다는 것이다.
>
...
**31초 근처의 측정 해상도가 5초다.** 14번째 폴과 15번째 폴 사이에 끝난 트랜잭션은 전부 15번째 폴에서야 발견된다.

다만 **눈금이 고정은 아니다.** 폴마다 HTTP 왕복이 `sleep` 위에 얹히므로 각 폴은 `공칭 시각 + 누적 왕복` 에 떨어진다. 실측 최댓값이 480 에서 32,217(+884ms), coroutine 1400 에서 32,968(**+1,635ms**)이다. 그만큼 14번째 폴도 26,333 이 아니라 28초 언저리까지 밀리므로, **"26.4초에 끝났는데 31.3초로 기록된다" 는 드리프트가 작을 때의 얘기다.**

드리프트가 회차 성격을 말해주지도 않는다. coroutine 1400 의 +1,635ms 가 1600(+711ms)보다 크다 — 왕복 잡음이라 회차마다 흔들린다.
...
**그러니 "이론 하한 31초" 자체가 눈금이었다.** 물리 바닥은 느린 은행의 **30초**다. 이 구간에서는 누적 왕복이 1.6초 아래라 14번째 폴이 28초 언저리에 떨어지고, 그러면 30초를 다 자는 트랜잭션은 **31,333 아래로 관측될 수가 없다.** 31.4초가 하한과 일치한 게 아니라 그 위 첫 눈금이 거기였다. 하한에 얹었다던 "오버헤드 1초" 는 재본 적이 없다.

(1800 은 다르다. 거기서는 왕복이 누적돼 14번째 폴이 30초를 넘겨 떨어지므로, 최솟값 30,019~30,089ms 가 **14번째 폴에서 관측된다.** 30초 바닥이 그제서야 보이는 셈이다.)
...
**2.4% 는 해석할 수 없는 값이다. 다만 이유는 잘림이 아니다.** 잘림은 느린 쪽을 버려 p95 를 **낮추는데**, 더 많이 잘린 팔(conn40k, 커버리지 84.4%)이 오히려 더 **느리게** 나왔다. 보정하면 차이가 좁아지는 게 아니라 벌어진다 — 이 편향은 2.4% 를 만들어내지 못한다.

**진짜 교란은 io1024 쪽 회차가 애초에 정상이 아니었다는 것이다.** k6 check 실패가 163건이고 `runs_total` 이 7,037 이다(형제 회차는 7,200~7,201). submit 이 202 를 못 받았다는 뜻이고, **그 팔은 부하를 2% 덜 받았다.** 2.4% 와 같은 방향이다. 그런데도 회차는 `valid:true` 로 통과했고 표 어디에도 안 남는다.
...
**워밍업** — 논블로킹 모드는 RPM 단계들을 **같은 JVM 위에서** 연속으로 돌릴 수 있다. 인자가 같아서 재기동이 인자 때문에 필요하진 않다. 다만 공짜는 아니다 — `bench.sh` 가 회차마다 `docker logs` 를 통째로 떠서 `/var/log/bench/` 에 남기므로(`run_id` 의 `/` 를 `_` 로 바꾼 이름, 예: `coroutine_default_rpm1800_rep1.log`), JVM 을 이어 쓰면 뒤 회차 로그에 앞 회차가 전부 섞인다. `bench.sh` 가 그 로그를 남기는 이유(회차별로 이상을 되짚기)가 회차 단위로 안 먹게 된다.
