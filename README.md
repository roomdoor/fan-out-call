...
| **coroutine** | IO 디스패처 **512** | 거부 0, 처리율 480.3/분 | **1,400 RPM** |
...
논블로킹 두 모드는 1,800 RPM에서도 7,200건을 **전부 처리했다.** 거부가 아니라 지연이 늘었을 뿐이다(31.4초 → 56.3 / 44.6초). 위 표의 한계는 "31.4초라는 같은 폴링 눈금에 머무는" 지점이고, 모드마다 다르다 — **coroutine 1,400 / webclient 1,600**. coroutine 은 1,600 에서 이미 일부가 다음 눈금으로 넘어갔다. 그 31.4초가 지연이 아니라 측정 눈금이라는 것도 2장 표 밑에 적었다.
