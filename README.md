...
**그리고 그 "이론 하한 31초" 자체가 눈금이었다.** 14번째 폴이 26,333 에 떨어지므로 **31,333 아래는 애초에 관측이 안 된다.** 31.4초가 하한과 일치한 게 아니라, 그 위쪽 첫 눈금이 거기였을 뿐이다. 하한에 얹었다던 "오버헤드 1초" 는 재본 적이 없다.
...
**1800 에서는 눈금이 아예 무너진다.** 요청 왕복이 초 단위로 늘어(회차별 985~1,968ms) 폴 수가 **13.78회**(84,911/6,163)로 정수가 아니게 된다. e2e 가 min 30,028 · med 38,594 · max 69,422 로 흩어지고 어느 눈금에도 안 걸린다.

**이 구간이 하필 무릎과 그 원인 후보를 지운 A/B 들이 사는 곳이다.** 그리고 여기서는 `e2e` 에 편향이 셋 더 얹힌다. 전부 실측으로 확인한 것만 적는다.
...
**첫 줄이 A/B 의 정밀도를 깬다.** 2.4% 를 읽은 coroutine 두 팔의 커버리지가 **84.4% 대 90.8%** 다. 한쪽의 "p95" 는 전체 분포로 치면 p80 언저리, 다른 쪽은 p86 언저리를 읽은 것이다.
