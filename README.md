...
**1600 근처까지는 눈금이 오히려 워밍업을 묶어준다.** 폴 수가 트랜잭션당 정확히 15.000 회다 — coroutine 480(28,635/1,909)·960(57,270/3,818)·1200(71,595/4,773)·1400(83,520/5,568), webclient 1400(83,520/5,568)·1600(95,445/6,363) 전부. **k6 가 기록한 트랜잭션은 예외 없이 15번째 폴에서 끝났다**는 뜻이고, 그러면 각각은 `(26,333, 31,333]` 안에서 끝난 것이다.

(여기도 잘림이 있다. DB 가 센 건수 대비 k6 기록이 480 에서 1,909/1,921, 1600 에서 6,363/6,400 — **0.6% 가 빠진다.** 빠지는 건 아래 표대로 제일 느린 것들이라, 이 묶음은 99.4% 에 대한 것이다.)
...
**그리고 그 "이론 하한 31초" 자체가 눈금이었다.** 14번째 폴이 26,333 에 떨어지므로, 30초를 다 자고 끝나는 트랜잭션은 **31,333 아래로 관측될 수가 없다.** 31.4초가 하한과 일치한 게 아니라, 그 위쪽 첫 눈금이 거기였을 뿐이다. 하한에 얹었다던 "오버헤드 1초" 는 재본 적이 없다.

(거부가 섞인 회차는 다르다. 거부된 호출은 즉시 끝나므로 async-threadpool 회차의 e2e 최솟값은 16,361~16,383ms — 12번째 폴(16,333) 눈금이다.)

격자가 무너진 1800 회차의 e2e 최솟값이 **30,028ms** 다. 이게 양자화 없이 본 첫 값이라 오버헤드가 1초보다 훨씬 작다는 건 알 수 있는데, **그 값도 폴로 본 시각이고 6,163개 중 최솟값**이며 접수 시간이 빠져 있다. 오버헤드의 **상한이 28ms** 라는 뜻이지 28ms 라고 잰 게 아니다.

**새기 시작하는 지점이 보인다.** coroutine 1600 은 15.0003 회(95,432/6,362)이고 e2e 최댓값이 38,188ms 다 — 몇 건이 이미 16번째 폴로 넘어갔다.

**1800 에서는 눈금이 아예 무너진다.** 요청 왕복이 100ms 단위로 늘어(중앙값 175~277ms) 눈금이 폴마다 그만큼씩 밀리고, 폴 수가 **13.78회**(84,911/6,163)로 정수가 아니게 된다 — 트랜잭션마다 다른 폴에서 끝난다는 뜻이다. e2e 가 min 30,028 · med 38,594 · max 69,422 로 흩어지고 어느 눈금에도 안 걸린다.

(산수가 맞는다. 최솟값 30,028 이 14번째 폴이라면 그 앞 14번의 왕복이 3,695ms 누적된 것이고, 한 번에 264ms 다 — 그 회차 중앙값 277ms 와 같다.)

**이 구간이 하필 무릎과 그 원인 후보를 지운 A/B 들이 사는 곳이다.** 그리고 여기서는 `e2e` 에 편향이 둘 더 얹힌다. 커밋된 데이터가 직접 말하는 것만 적는다.

| 편향 | 실측 | 방향 |
| --- | --- | --- |
| **끝에서 잘린다** | k6 가 e2e 를 남긴 건수가 DB 가 센 것의 **84.4~96.1%** 다. `gracefulStop` 에 잘린 iteration 은 지표에 안 들어가는데, 잘리는 건 **제일 오래 걸린 것들**이다 | 낮게 |
| **접수 시간이 빠진다** | `startTime` 을 submit 이 202 를 돌려준 뒤에 잡는다 | 낮게 |

**첫 줄이 A/B 의 정밀도를 깬다.** 2.4% 를 읽은 coroutine 두 팔의 커버리지가 **84.4% 대 90.8%** 다. 같은 "p95" 라도 서로 다른 크기의 분포에서 읽은 값이라, 두 수를 뺀 2.4% 가 무엇의 차이인지 말할 수 없다.

**다만 방향은 결론과 반대다.** io1024 쪽이 느린 꼬리를 **더 많이 남기고도** 더 낮게 나왔다(54,419 대 55,766). 잘림을 보정하면 차이가 **좁아지는 게 아니라 벌어진다.** 그러니 이 편향이 2.4% 라는 숫자를 만들어낸 건 아니고, 그 숫자를 몇 %라고 말할 근거를 없앨 뿐이다.

그리고 **그 회차 중 하나는 애초에 정상이 아니었다.** io1024 coroutine 은 `runs_total` 이 7,037 로 나머지 1800 회차(7,200~7,201)보다 164 적고, k6 check 실패가 163건이다 — submit 이 202 를 못 받았다는 뜻이다. 그런데 회차는 `valid:true` 로 통과했고 표 어디에도 안 남는다.

> 앞에 쓴 요청 왕복(985~1,968ms)은 `http_req_duration` **평균**이라 느린 submit 몇 건이 끌어올린 값이다. 중앙값은 conn40k 175/277ms, io1024 **21/28ms** 로 **순서가 뒤집힌다.** 전형적인 폴 간격은 5.0~5.3초에 가깝다. 폴만 따로 세는 지표가 없어 둘 다 적어둔다.
