...
논블로킹 두 모드는 1,800 RPM에서도 7,200건을 **전부 처리했다.** 거부가 아니라 지연이 늘었을 뿐이다(31.4초 → 56.3 / 44.6초). 위 표의 1,600은 "31.4초라는 같은 폴링 눈금에 머무는" 한계다 — 그 31.4초가 지연이 아니라 측정 눈금이라는 건 2장 표 밑에 적었다.
...
**1600까지 같은 폴링 눈금에 머물다가 1800에서 꺾인다.** 전 구간 거부 0이다.
...
>
> **측정 편향.** 이 표의 1.0%·16% 와 아래 2.4% 는 전부 **1800 RPM 회차**(p95 44~56초)에서 나온 값인데, 그 구간의 `e2e` 는 폴링 눈금 위에 있고 회차 끝에서 잘린다 — k6 가 남긴 건수가 DB 가 센 것의 **84.4~96.1%** 이고, **잘리는 건 제일 느린 것들**이다. 2.4% 를 읽은 두 팔은 커버리지가 84.4% 대 90.8% 로 서로 다르다. 게다가 io1024 coroutine 회차는 submit 실패 163건으로 `runs_total` 이 164 적은데도 `valid:true` 로 통과했다.
>
...
**1400 까지는 모든 회차가 눈금 하나에 뭉친다.** 폴 수를 기록 건수로 나누면 정확히 15.000 이다 — 두 모드의 480·960·1200·1400 과 webclient 1600, 아홉 회차 전부. 그 회차들의 e2e 최솟값이 31,333 위이고 최댓값이 36,333 아래라, **기록된 트랜잭션이 전부 15번째 폴에서 끝났다**는 뜻이다. 각각은 `(26,333, 31,333]` 안에서 끝났다.
...
**2.4% 는 해석할 수 없는 값이다.** 그걸 읽은 coroutine 두 팔의 커버리지가 **84.4% 대 88.7%** 로 다르고(같은 분모 7,201 기준), 게다가 **io1024 쪽 회차는 애초에 정상이 아니었다** — `runs_total` 이 7,037 로 형제 회차(7,201)보다 164 적고 k6 check 실패가 163건이다. submit 이 202 를 못 받았다는 뜻이고, **그 팔은 부하를 2% 덜 받은 것**이다. 2.4% 라는 차이와 같은 방향의 교란이다. 그런데도 회차는 `valid:true` 로 통과했고 표 어디에도 안 남는다.
