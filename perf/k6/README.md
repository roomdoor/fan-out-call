...
k6는 `DURATION` 에서 멈추지만 트랜잭션 e2e 이론 하한이 31초다(은행 50곳 중
2곳이 30초). v15 에서 거부가 없던 회차들의 실측 최솟값은 30.0~31.4초였다.
...
- `e2e_completion_time` — submit 이 **202 를 돌려준 뒤부터**, 폴링이 종료 상태를 확인할 때까지.
  접수 시간은 안 들어가고, 다음 폴 시각까지 위로 올림된다(간격은 100ms 에서 1.5배씩,
  5,000ms 상한). 그 눈금은 고정이 아니라 **누적된 폴 왕복만큼 위로 밀린다** —
  실측으로 +29ms 에서 +1,635ms 까지 봤다. 31초 근처에서는 눈금 간격이 5초라
  그 안의 차이가 안 보인다 — 저장소 루트 README 의
