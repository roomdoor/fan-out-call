...
// 무효 회차는 표에서 뺀다. 숫자를 못 냈거나 못 믿을 회차라서, 섞으면
// 평균이 조용히 낮아진다. 대신 아래에 몇 건인지 적는다.
...
// 반복 측정의 흩어짐을 숨기지 않는다. n=1로 "185는 되고 200은 안 된다"를
// 단정했던 것이 이 프로젝트가 겪은 문제다.
function spread(xs) {
  const v = num(xs);
...
const header = [
  ...
  'FAILED(포화)', 'FAILED(오류)', '거부된 은행콜', '처리율/분', 'e2e p95 (ms)', 'timeout',
];
...
// 다섯 버킷 합이 전체 행 수와 달랐던 회차. 셈에서 빠진 게 있다는 뜻이라
// 그 회차 숫자는 못 믿는다.
if (runs.some((r) => !r.balanced)) {
  notes.push('⚠️ 은행콜 버킷 합이 전체 행 수와 다른 회차가 있다. 분류에서 빠진 응답 코드가 있다는 뜻이다.');
}
...
// k6가 도착률을 못 맞추면 부하가 덜 걸린 것인데, 표에서는 천장에 닿은 것과
// 구분되지 않는다. 측정 도구가 병목이면 그 회차는 게이트웨이 수치가 아니다.
