...
class LoanLimitQueryOrchestrator(
    ...
    fun query(
        ...
        fanOutExecutor: LenderFanOutExecutor,
        modeName: String,
        ...
        log.info(
            "Loan-limit query accepted runId=$runId transactionId=${runEntity.requestId} " +
                "borrowerId=${request.borrowerId} lenderCount=${lenders.size} mode=$modeName",
        )
    ...
    private suspend fun processInBackground(
        ...
        try {
            log.info(
                "Background fan-out started runId=$runId borrowerId=${request.borrowerId} " +
                    "lenderCount=${lenders.size} mode=$modeName",
            )
